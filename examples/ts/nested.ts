...
const findAndFollowTransitions = [
  ...
  getTransition("followToExit", FollowEntity, Exit)
...
const FollowMachine =   getNestedMachine("findAndFollow", findAndFollowTransitions, FindEntity, Exit)
                          .setBuildArgs(playerFilter)
                          .build();

const rootTransitions = [
  getTransition("idleToFind", Idle, FindEntity)
    .setBuildArgs(playerFilter)
    .build(),

  getTransition("findToLook", FindEntity, LookAtTarget)
    .build(),

  getTransition("lookToIdle", LookAtTarget, Idle)
    .build(),

  getTransition("findToTest", FindEntity, FollowMachine)
    .build(),

  getTransition("testToIdle", FollowMachine, Idle)
    .setShouldTransition(isFinished)
    .build(),
];
